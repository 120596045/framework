...
public class MyApplyController {
	...
	public String reimburseapplymanage() {
		...
		session.setAttribute("category_dispatchs", category_dispatchs);
		Professional Professional =(Professional) SecurityUtils.getSubject().getSession().getAttribute("professional");
	...
	public String applyexamine(Integer uId, Integer aId) {
		Session session=SecurityUtils.getSubject().getSession();
		...
		if(applies3!=null){
			for (Apply apply3 : applies3) {
				if(apply3.getaId()==aId){
					session.setAttribute("APPLY", apply3);
					return "apply/applyexamine";
				}
			}
		}
		if(applies2 !=null){
			for (Apply apply2 : applies2) {
				if(apply2.getaId()==aId){
					session.setAttribute("APPLY", apply2);
					return "apply/applyexamine";
				}
			}
		}
